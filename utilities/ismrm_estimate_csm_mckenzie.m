function [csm] = ismrm_estimate_csm_mckenzie(img, percent)
%
...
% scale_correction = (sum(abs(img_matrix).^2,2));
% scale_correction (scale_correction < eps) = 1;
% scale_correction= sqrt(scale_correction);
% % nonzero_ind = scale_correction > 0;
% nonzero_ind = scale_correction > percent*min(scale_correction(:));
...
img_matrix = reshape(img, [nx*ny nc]);
scale_correction = (sum(abs(img_matrix).^2,2));
nonzero_ind = scale_correction > percent*max(scale_correction(:));
scale_correction= sqrt(scale_correction);
